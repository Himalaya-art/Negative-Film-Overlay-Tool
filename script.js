...
selectAllCheckbox.addEventListener('change', (e) => {
    ...
    checkboxes.forEach(checkbox => {
        ...
        if (listItem) {
            const previewImg = listItem.querySelector('img');
            if (previewImg) {
                previewImg.classList.toggle('inverted-preview', e.target.checked);
            }
        }
    });
    redrawCanvas();
...
// 将复选框放在标签内部，这样可以应用CSS样式
selectAllLabel.prepend(selectAllCheckbox);  // 将复选框添加到标签内部的前面
